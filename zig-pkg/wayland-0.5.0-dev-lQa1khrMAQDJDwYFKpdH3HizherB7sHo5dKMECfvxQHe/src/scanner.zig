...
pub fn main(init: std.process.Init.Minimal) !void {
    ...
    const threaded_io = std.Io.Threaded.global_single_threaded.io();
    const out_file = try std.Io.Dir.createFileAbsolute(threaded_io, out_path, .{});
    defer out_file.close(threaded_io);

    var out_buf: [4096]u8 = undefined;
    var out_writer = out_file.writer(threaded_io, &out_buf);
...
const Scanner = struct {
    ...
    const Map = std.StringArrayHashMapUnmanaged(io.Writer.Allocating);
    client: Map = .empty,
    server: Map = .empty,
    common: Map = .empty,
    ...
    fn scanProtocol(scanner: *Scanner, xml_path: []const u8) !void {
        const threaded_io = std.Io.Threaded.global_single_threaded.io();
        const xml_file = try std.Io.Dir.cwd().openFile(threaded_io, xml_path, .{});
        defer xml_file.close(threaded_io);

        var arena = std.heap.ArenaAllocator.init(gpa);
        defer arena.deinit();

        var read_buf: [4096]u8 = undefined;
        var xml_reader = xml_file.reader(threaded_io, &read_buf);
        const xml_bytes = try xml_reader.interface.allocRemaining(arena.allocator(), .limited(512 * 4096));
...
const Protocol = struct {
    ...
    fn parse(arena: mem.Allocator, parser: *xml.Parser) !Protocol {
        ...
        var interfaces: std.StringArrayHashMapUnmanaged(Interface) = .empty;
    ...
    fn find_globals(arena: mem.Allocator, interfaces: std.StringArrayHashMapUnmanaged(Interface)) ![]const Global {
        ...
        for (interfaces.values()) |interface| {
            if (!non_globals.contains(interface.name)) {
                var children: std.StringArrayHashMapUnmanaged(Interface) = .empty;
    ...
    fn find_children(
        parent: Interface,
        interfaces: std.StringArrayHashMapUnmanaged(Interface),
        children: *std.StringArrayHashMapUnmanaged(Interface),
