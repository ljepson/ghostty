...
pub fn add(
    ...
    if (step.rootModuleTarget().os.tag == .linux) {
        ...
        if (std.Io.Dir.cwd().access(std.Io.Threaded.global_single_threaded.io(), path, .{})) {
    ...
    step.root_module.addCSourceFile(.{ .file = b.path("src/stb/stb.c"), .flags = if (step.rootModuleTarget().abi == .msvc)
    ...
    // In Zig 0.16.0, C++ linking is automatic when C++ source files are added.
