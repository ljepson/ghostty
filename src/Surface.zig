...
pub fn init(
    ...
    self.renderer_thr.setName(stdIo(), "renderer") catch {};
    ...
    self.io_thr.setName(stdIo(), "io") catch {};
...
pub fn activateInspector(self: *Surface) !void {
    ...
    {
        self.renderer_state.mutex.lockUncancelable(stdIo());
        defer self.renderer_state.mutex.unlock(stdIo());
    ...
    switch (msg) {
        ...
        .ring_bell => bell: {
            ...
            if (self.last_bell_time) |last| {
                if (durationSince(last, now) < 100 * std.time.ns_per_ms) break :bell;
            }
        ...
        .start_command => {
            self.command_timer = nowTimestamp();
        },
        ...
        .stop_command => |v| timer: {
            ...
            const duration: Duration = .{ .duration = durationSince(start, end) };
...
fn resolvePathForOpening(
    ...
    if (!std.fs.path.isAbsolute(path)) {
        ...
        std.Io.Dir.accessAbsolute(stdIo(), resolved, .{ .read = true }) catch {
...
fn mouseShiftCapture(self: *const Surface, lock: bool) bool {
    ...
    if (lock) self.renderer_state.mutex.lockUncancelable(stdIo());
    defer if (lock) self.renderer_state.mutex.unlock(stdIo());
...
pub fn mouseButtonCallback(
    ...
    if (button == .left and action == .press) {
        ...
        extend_selection: {
            ...
            const now = nowTimestamp();
            const since = durationSince(self.mouse.left_click_time, now);
    ...
    switch (action.scoped(.surface).?) {
        ...
        .search => |text| search: {
            const s: *Search = if (self.search) |*s| s else init: {
                ...
                s.thread.setName(stdIo(), "search") catch {};
...
fn writeScreenFile(
    ...
    var file = try tmp_dir.dir.createFile(
        ...
        switch (builtin.os.tag) {
            .windows => .{},
            else => .{ .permissions = @enumFromInt(0o600) },
        },
    );
    defer file.close(io);
    ...
    const path_len = try tmp_dir.dir.realPathFile(io, filename, &path_buf);
...
fn showDesktopNotification(self: *Surface, title: [:0]const u8, body: [:0]const u8) !void {
    ...
    if (self.app.last_notification_time) |last| {
        if (durationSince(last, now) < 1 * std.time.ns_per_s) {
    ...
    if (self.app.last_notification_time) |last| {
        if (self.app.last_notification_digest == new_digest) {
            if (durationSince(last, now) < 5 * std.time.ns_per_s) {
